...
async def test_complete_workflow_with_file_system_simulation() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert "digital wandering" in diary_obs.brief_note or "Chapter 1" in str(diary_obs.preview or "")
...
async def test_complete_workflow_with_real_llm_api() -> None:
    ...
    assert any(word in content for word in ["config", "letter", "key", "hidden", "secret"])
